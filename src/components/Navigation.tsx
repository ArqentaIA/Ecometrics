  ...
  return (
    <nav className="sticky top-0 z-50 bg-nav text-nav-foreground"
      ...
      <div className="max-w-7xl mx-auto px-5 h-12 flex items-center justify-between">
        ...
        {/* Center: Nav links (Windows 11 Pivot style) */}
            ...
            return (
              <Link
                ...
                className={`relative h-full flex items-center px-4 text-[13px] transition-all duration-150 ${
                  isActive
                    ? "text-nav-foreground font-semibold"
                    : "text-nav-foreground/60 hover:text-nav-foreground/90 hover:bg-nav-foreground/5"
                }`}
              >
                {l.label}
                {isActive && (
                  <span className="absolute bottom-0 left-1/2 -translate-x-1/2 w-5 h-[3px] bg-nav-foreground rounded-t-full" />
                )}
              </Link>
            );
          })}
